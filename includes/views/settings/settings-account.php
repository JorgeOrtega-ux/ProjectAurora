...
if (isset($_SESSION['user_id'])) {
    ...
    if (isset($dbConnection)) {
        $stmt = $dbConnection->prepare("SELECT language, open_links_new_tab FROM user_preferences WHERE user_id = :id");
        $stmt->execute([':id' => $_SESSION['user_id']]);
        $dbPrefs = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($dbPrefs) {
            $prefs['language'] = $dbPrefs['language'];
            $prefs['open_links_new_tab'] = (bool)$dbPrefs['open_links_new_tab'];
        }
    }
}
            ...
            <div class="component-group-item">
                ...
                <div class="component-card__actions actions-right">
                    <label class="component-toggle-switch">
                        <input type="checkbox" id="pref-open-links" <?= $checkedAttr ?>>

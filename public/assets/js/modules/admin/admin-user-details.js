...
async function loadData() {
    // [REFACTORIZADO]
    const data = await AdminApi.getUserDetails(targetId);
...
async function saveRoleChanges() {
    ...
    // [REFACTORIZADO]
    const res = await AdminApi.updateUserRole(targetId, newRole);
...
async function saveStatusSanction() {
    ...
    const durationDays = (statusType === 'suspended_perm') ? 'permanent' : parseInt(duration);
    
    // [REFACTORIZADO]
    const res = await AdminApi.updateUserStatus(targetId, 'suspended', reason, durationDays);
...
async function liftBan() {
    ...
    // [REFACTORIZADO]
    const res = await AdminApi.updateUserStatus(targetId, 'active');
...
async function saveManageChanges() {
    ...
    // [REFACTORIZADO]
    const res = await AdminApi.updateUserGeneral(payload);

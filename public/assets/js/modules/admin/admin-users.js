...
function setupActionButtons(uid) {
    ...
    if (btnEdit) btnEdit.onclick = () => uid && window.navigateTo('admin/user-edit?uid=' + uid);
}
    ...
    try {
        ...
        if (data.html_rows !== undefined) {
            if (tbody) {
                ...
                // [NUEVO] Inyectar atributo data-img-type="user" a las imágenes cargadas por PHP
                // Esto permite que el sistema de fallback (app-init.js) las detecte
                tbody.querySelectorAll('.capsule-avatar img').forEach(img => {
                    img.setAttribute('data-img-type', 'user');
                });
            }

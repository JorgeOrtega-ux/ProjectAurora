// public/assets/js/modules/auth-manager.js
...
async function handleResendCode(type, linkSelector) {
    ...
    const res = await AuthApi.resendCode(type);
...
export function initAuthManager() {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('[data-action="register-step1"]')) {
            e.preventDefault();
            await handleRegisterStep('step1', 2, 'register/additional-data');
        }
        if (e.target.closest('[data-action="register-step2"]')) {
            e.preventDefault();
            const success = await handleRegisterStep('step2', 3, 'register/verification-account');
            if (success) initResendTimer('[data-action="resend-register"]');
        }
        if (e.target.closest('[data-action="register-step3"]')) {
            e.preventDefault();
            await handleRegisterStep('step3', 'main', null);
        }
...
async function handleRegisterStep(stepName, nextStep, nextUrl) {
    let apiPromise = null;
    ...
    if (stepName === 'step1') {
        ...
        // [REFACTORIZADO]
        apiPromise = AuthApi.registerStep1(emailVal, passVal);
        ...
        // [REFACTORIZADO]
        apiPromise = AuthApi.registerStep2(userVal);
        ...
        const cleanCode = codeIn.value.replace(/-/g, '');
        
        // [REFACTORIZADO]
        apiPromise = AuthApi.verifyRegisterCode(cleanCode);
    ...
    // [REFACTORIZADO] Llamada a la nueva función con la promesa
    return await executeAuthStep(apiPromise, btnSelector, errorSelector, nextStep, nextUrl);
}
...
async function handleRecoveryLinkRequest() {
    ...
    const res = await AuthApi.recoveryStep1(emailVal);
...
async function handleResetPasswordFinal() {
    ...
    const res = await AuthApi.recoveryFinal(tokenVal, passVal, passConfirmVal);
...
// [NUEVO] Función auxiliar para ejecutar la promesa de la API y manejar la UI
async function executeAuthStep(apiPromise, btnSelector, errorSelector, nextStep, nextUrl) {
    ...
    try {
        ...
        if (result.success) {
            if (nextStep === 'main') {
                ...
                // Si es el paso 2 (que lleva al 3), mostrar alerta de código enviado
                if (nextStep === 3 && window.alertManager) {
...
async function handleLogin() {
    ...
    const res = await AuthApi.login(emailInput.value.toLowerCase(), passInput.value);
...
async function handleLogin2FA() {
    ...
    const res = await AuthApi.verifyLogin2FA(cleanCode);

...
import { setButtonLoading, toggleCardError, qs } from '../core/utilities.js';
...
function initDeleteAccountLogic() {
    ...
    confirmBtn.onclick = async () => {
        ...
        const res = await SettingsApi.deleteAccount(password);
...
async function initSessionsLogic() {
    ...
    container.addEventListener('click', async (e) => {
        ...
        if (btn) {
            ...
            const res = await SettingsApi.revokeSession(sessionIdDb);
    ...
    if (revokeAllBtn) {
        revokeAllBtn.onclick = async () => {
            ...
            const res = await SettingsApi.revokeAllSessions();
...
function initChangePasswordLogic() {
    ...
    verifyBtn.onclick = async () => {
        ...
        const res = await SettingsApi.verifyPassword(pass);
    ...
    saveBtn.onclick = async () => {
        ...
        const res = await SettingsApi.updatePassword(newPass, logout);
...
function initBooleanPreferencesLogic() {
    document.body.addEventListener('change', async (e) => {
        ...
        if (target.matches('input[type="checkbox"][data-preference-type="boolean"]')) {
            ...
            const res = await SettingsApi.updateBooleanPreference(fieldName, isChecked);
...
function initPreferencesLogic() {
    document.body.addEventListener('click', async (e) => {
        ...
        if (prefType === 'usage') {
            apiPromise = SettingsApi.updateUsage(value);
        } else if (prefType === 'language') {
            apiPromise = SettingsApi.updateLanguage(value);
        } else if (prefType === 'theme') {
            apiPromise = SettingsApi.updateTheme(value);
        } else if (prefType === 'message_privacy') {
            apiPromise = SettingsApi.updatePrivacy(value);
        } else {
...
function initProfilePictureLogic() {
    ...
    elements.saveBtn.onclick = async () => {
        ...
        try {
            const res = await SettingsApi.updateProfilePicture(file);
            if (res.success) {
                if (window.alertManager) window.alertManager.showAlert(res.message, 'success');
    ...
    elements.removeBtn.onclick = async () => {
        ...
        const res = await SettingsApi.removeProfilePicture();
...
function initUsernameLogic() {
    ...
    els.saveBtn.onclick = async () => {
        ...
        const res = await SettingsApi.updateUsername(newVal);
...
function initEmailLogic() {
    ...
    els.saveBtn.onclick = async () => {
        ...
        const res = await SettingsApi.updateEmail(newVal);
...
function initTwoFactorLogic() {
    ...
    if (els.verifyBtn) {
        els.verifyBtn.onclick = async () => {
            ...
            const res = await SettingsApi.verifyPassword(password);
    ...
    if (els.confirmBtn) {
        els.confirmBtn.onclick = async () => {
            ...
            const res = await SettingsApi.enable2FA(tempSecret, code);
    ...
    if (els.disableBtn) {
        els.disableBtn.onclick = async () => {
            ...
            const res = await SettingsApi.disable2FA(password);
    ...
    async function generateSecret(els) {
        const res = await SettingsApi.generate2FASecret();

...
import { FriendApi } from '../../services/api-service.js';
...
async function blockUser(targetId, btn) {
    ...
    const res = await FriendApi.blockUser(targetId);
...
async function unblockUser(targetId, btn) {
    ...
    const res = await FriendApi.unblockUser(targetId);
...
async function respondRequest(actionType, btn, senderId) { 
    ...
    try {
        ...
        if (actionType === 'accept_request') {
            res = await FriendApi.acceptRequest(senderId);
        } else {
            res = await FriendApi.declineRequest(senderId);
        }
...
async function startDirectChat(targetUid) {
    const res = await FriendApi.startChat(targetUid);

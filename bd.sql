...
CREATE TABLE IF NOT EXISTS users (
    ...
    last_online_at TIMESTAMP NULL -- [RECOMENDADO] Para optimizar dashboard
);
...
CREATE TABLE IF NOT EXISTS communities (
    ...
    max_members INT DEFAULT 0, -- [NUEVO] 0 = ilimitado
    status ENUM('active', 'maintenance') DEFAULT 'active', -- [NUEVO] Estado de la comunidad
    profile_picture VARCHAR(255) NULL,
...
-- TABLA DE CANALES (MODIFICADA PARA SOPORTE DE VOZ Y MANTENIMIENTO)
CREATE TABLE IF NOT EXISTS community_channels (
    ...
    status ENUM('active', 'maintenance') DEFAULT 'active', -- [NUEVO] Estado del canal
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS community_channel_reads (
    ...
    last_read_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_read (user_id, channel_id),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (community_id) REFERENCES communities(id) ON DELETE CASCADE,
...
INSERT IGNORE INTO communities (uuid, community_name, community_type, access_code, privacy, member_count, max_members, status, profile_picture, banner_picture, is_verified) VALUES
('comm-uuid-001', 'Universidad Autónoma de Nuevo León', 'university', 'UANL-STUD-2025', 'public', 120, 0, 'active', 'https://ui-avatars.com/api/?name=UANL&background=F7C700&color=000', 'https://placehold.co/600x200/F7C700/000000?text=UANL', 1),
('comm-uuid-002', 'Tecnológico de Monterrey', 'university', 'ITESM-TEC-2025', 'public', 890, 0, 'active', 'https://ui-avatars.com/api/?name=TEC&background=0033A0&color=fff', 'https://placehold.co/600x200/0033A0/ffffff?text=Borregos+TEC', 1),
